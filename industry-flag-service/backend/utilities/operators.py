...
class op:
    ...
    def to_json_file(source_object, json_file_path, file_open_mode="w", indent=2):
        tmp_json_string = op.to_json(
            source_object=source_object, indent=indent)
        op.write_to_file(
            data=tmp_json_string, filePath=json_file_path, openMode=file_open_mode, end="")
    ...
    def load_file(inputFile):
        buffer = io.BytesIO(open(inputFile, "rb").read())
        return buffer
